...
```{r}
p +
  geom_point(aes(color = thermoregulation), size = 2)
```

We can also map shapes to another categorical variable.
```{r}
p +
  geom_point(aes(shape = thermoregulation, color = thermoregulation), size = 2)
```
...
p +
  geom_point(
    size = 3, # Border weight of points
    color = "black", # Border color of points
    alpha = 0.8, # Transparency
    shape = 24, # Shape of points
    fill = "lemonchiffon" # Fill color of points. Note that not all shapes accept all colors
    )
```
...
homerange %>% 
  ggplot(aes(x = taxon, fill = trophic_guild)) +
  geom_bar(position = "dodge") + # Splits bars by trophic guild
  coord_flip() +
  labs(
    title = "Observations by Taxon",
    x = "Taxonomic Group",
...
homerange %>% 
  ...
  theme(axis.text.x = element_text(
    angle = 60, # Tilt x labels by 60 degrees
    hjust = 0.85 # Shift x label horizontally by 0.85
    )
  ) +
  labs(
    title = "Observations by Taxon",
...
homerange %>% 
  ...
  geom_boxplot()
# Group by taxa, but des not specify what taxa per boxplot
```
