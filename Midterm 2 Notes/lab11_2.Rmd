...
q <- homerange %>% 
  ...
  labs(title = "Predator Mass vs. Prey Mass",
       x = "Predator Mass (Log10 g)",
       y= "Prey Mass (Log10 g)"
       )
```
...
```{r}
my_palette <- paletteer_d("rtist::warhol")
my_palette
q + theme_classic() +
...
homerange %>% 
  ...
  geom_density(fill = "blue", alpha = 0.3)+
  facet_grid(thermoregulation~locomotion, labeller = label_both) +
  labs(
    title = "Distribution of Homerange Size by Locomotion and Thermoregulation",
    x = "Homerange Size",
    y = "Density"
  )+
