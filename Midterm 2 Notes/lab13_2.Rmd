...
```{r}
anti_join(table_A, table_B, by = "customer_ID") # Best for troubleshooting problematic joins
...
```{r message=FALSE, warning=FALSE}
spiders <- read.csv("data/spider_records.csv") %>% clean_names()
summary(spiders)
```

```{r, message = FALSE}
locations <- read.csv("data/spiders_locations.csv") %>% clean_names()
summary(locations)
```

2. Join the files using an appropriate key and join type.
```{r}
left_join(spiders, locations, by = "accession")
```

```{r}
spiders_and_locations <- inner_join(spiders, locations, by = "accession")
```

3. Convert the date column to a date format using lubridate.
```{r}
spiders_and_locations.csv <- spiders_and_locations %>% 
  mutate(date = dmy(date))
```

4. Save your final data to a `.csv`.
```{r}
spiders_and_locations.csv <- 
```
