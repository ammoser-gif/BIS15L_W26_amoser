...
```{r message=FALSE, warning=FALSE}
life_history <- read_csv("data/mammal_lifehistories_v2.csv", na="-999") %>% # "na = -999" replaces NA with a value of -999
  clean_names()
```
...
life_history %>% 
  count(order, sort=T) %>% 
  ggplot(aes(x = n, y = order, fill = order)) +
  geom_col()
```
...
life_history %>% 
  ...
  ggplot(aes(x = mean_mass, y = order, fill = order)) +
  geom_col()
```
...
life_history %>% 
  ...
  ggplot(aes(y = mean_mass, x = order, fill = order)) +
  geom_col() +
  coord_flip() +
  scale_y_log10()
```
...
life_history %>% 
  ...
  summarise(mean_mass = mean(mass, na.rm=T)) %>% 
  arrange(desc(mean_mass)) %>% 
  ggplot(aes(x = reorder(order, mean_mass), y = mean_mass, fill = order)) +
  geom_col() +
  coord_flip() +
  scale_y_log10()
```
...
life_history %>% 
  ...
  theme(plot.title = element_text(size = rel(2), hjust = .5)) # Change title size to 1.5 and shift horizontal orientation to middle
```
