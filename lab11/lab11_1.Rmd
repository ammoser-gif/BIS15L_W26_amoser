...
gap %>% 
  ...
  labs(
    title = "GDP per Capita in the USA, 2000-2025",
    x = "Year",
    y = "GDP"
  )
```
...
gap %>% 
  ...
  labs(
    title = "Distribution of Life Expectancy, 1930 and 1950",
    x = "Life Expectancy",
    y = NULL,
    color = "Year",
    fill = "Year"
...
income_cat2000 %>% 
  ...
  group_by(income_category) %>% 
  summarise(countries = n()) %>% 
  ggplot(aes(x = income_category, y = countries, fill = income_category)) +
  geom_col() +
  labs(
    title = "Number of Countries per Income Category",
    y = NULL
  ) + 
