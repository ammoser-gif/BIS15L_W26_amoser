...
```{r}
life_history %>% 
  summarise(across(everything(), ~ sum(is.na(.==-999))))
```
Here we look for all of the -999's and NA's
```{r}
life_history %>% 
  summarise(across(everything(), ~ sum(.==-999 | is.na(.))))
```
...
```{r}
life_history_no_nas %>% 
  summarise(across(everything(), ~ sum(.==-999)))
```
...
```{r message=FALSE, warning=FALSE}
cities <- read.csv("data/cites.csv") %>% clean_names()
glimpse(cities)
summary(cities)
```
