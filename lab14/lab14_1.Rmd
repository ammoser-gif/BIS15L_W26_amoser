...
```{r}
map1 <- get_stadiamap(bbox, maptype = "stamen_terrain", zoom = 8) # Higher zoom = More detail, Lower Zoom = Lower quality
```
...
ggmap(map1) +
  ...
  labs(x = "Longitude", y = "Latitude", title = "California, Usofila Records")
```
...
grizzly %>% 
  summary()
lat_grizz <- c(55.02, 70.37)
long_grizz <- c(-166.2, -131.3)
bbox_grizz <- make_bbox(long_grizz, lat_grizz, f = 0.03)
```

3. Load a map from `stadiamaps` in a `stamen_terrain` projection and display the map.
```{r}
map_grizz <- get_stadiamap(bbox_grizz, maptype = "stamen_terrain", zoom = 5) # Higher zoom = More detail, Lower Zoom = Lower quality
```
...
```{r}
ggmap(map_grizz) +
  geom_point(data = grizzly, aes(longitude, latitude), size = 0.75, color = "blue", alpha = 0.8) +
  labs(x = "Longitude", y = "Latitude", title = "Alaskan Bear Sightings")
```
