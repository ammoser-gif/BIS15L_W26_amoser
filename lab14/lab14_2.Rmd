...
```{r echo=FALSE, message=FALSE, warning=FALSE}
homerange <- read.csv("data/Tamburelloetal_HomeRangeDatabase.csv") %>%  clean_names()
```

```{r}
# Desired plot
homerange %>% 
  ggplot(aes(x = locomotion, fill = trophic_guild)) + # Allow user to swithc fill option between trophic_guild and thermoregulation
...
ui <- fluidPage(
  radioButtons("fill",
              "Select Trophic Guild or Thermoregulation",
...
server <- function(input, output, session) {
  output$plot <- renderPlot({
    ggplot(data = homerange,
           aes_string(x = "locomotion", fill = input$fill)) +
      geom_bar(position = "dodge")
  })
}
