...
Default shows first 10 lines of txt files
```
...
grep ">" e.coli_genbank.fasta -> get lines in file that start with >
can do multiple files at once
grep ">" e.coli_genbank.fasta e.coli_refseq.fasta -> get lines in both files that start with >
```
...
```{}
less e.coli_cds.fasta
```
thrL, thrA, thrB, etc.
4. Display at least five of the genes found in ecoli. 
```{}
grep ">" e.coli_cds.fasta > e.coli_protein_names.txt
head e.coli_protein_names.txt
```
