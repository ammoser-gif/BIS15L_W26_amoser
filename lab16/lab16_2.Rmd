...
3. Select the folder that contains our query sequence, **unknown_genome_01.txt** or paste the sequence in the **Enter Query Sequence** text box. 
...
```{r}
genome01_blastn <- read_csv("data/blast_results/genome01_blast.csv") %>% clean_names()
```

```{r}
glimpse(genome01_blastn)
```
...
```
\
The top 5 hits are **Candidatus Vidania fulgoroideorum, Candidatus Vidania fulgoroideorum, Candidatus Vidania fulgoroideorum, endosymbiont of Euscepes postfasciatus, and Candidatus Vidania fulgoroideorum**.
\
...
```
\
The hit with the highest E-value is **Candidatus Shikimatogenerans sp. Tcar**.
\
...
```{r}
dna_file <- readDNAStringSet("data/e.coli_genbank.fasta")
```
...
```{r}
aa_file <- readAAStringSet("data/e.coli_cds.fasta")
```
...
```{}
mv blastp_results.csv BIS15L_W26_amoser/lab16/data/blast_results/
```

2. Read the csv file into R and clean the column names. Save the cleaned data frame as `blastp_results`.

```{r}
blastp_results <- read_csv("data/blast_results/blastp_results.csv") %>% clean_names()
```
