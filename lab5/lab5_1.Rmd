...
```{r}
glimpse(penguins) # Best when data is MESSY
```

```{r}
summary(penguins) # Best when data is CLEAN
```
...
```{r}
names(penguins) # Show variable names
```
...
```{r}
select(penguins, species:flipper_length_mm) # Select columns from species to flipper_length_mm, inclusive
```
...
```{r}
select(penguins, contains("mm")) # Select columns using "mm" as a measurement
```
...
```{r}
select(penguins, where(is.numeric)) # Select columns that contain only numeric data
```
...
```{r}
rename(penguins, body_mass=body_mass_g) # TEMPORARLY rename "body_mass_g" as "body_mass"
```
...
```{r}
relocate(penguins, year) # Move "year" column to the front
```
