...
```{r}
filter(penguins, body_mass_g %in% c(5250, 5500)) # Filter for penguins whose body mass is either 5250 g or 5500 g
```

Alternatively, you can use `between` if you are looking for a range of specific values.  
```{r}
filter(penguins, between(body_mass_g, 5250, 5500)) # Filter for penguins whose body mass is between 5250 g and 5500 g
```

How would you use `%in%` to get the same result?
```{r}
filter(penguins, body_mass_g %in% c(5250, 5400, 5300, 5350, 5500))
```

You can also extract observations "near" a certain value but you need to specify a tolerance.  
```{r}
filter(penguins, near(body_mass_g, 5400, tol=100)) # Filter for penguins whose body mass is within 5300 and 5500
```
...
```{r}
filter(penguins, species=="Chinstrap" & body_mass_g > 4500) # Filter for penguins where are Chinstrap AND have a body mass greater than 4500 g
```
Note that "&" is the same as "," in some cases.

Notice that the `|` operator generates a different result. Why?
```{r}
filter(penguins, species =="Chinstrap" | body_mass_g > 4500) # Filter for penguins who are Chinstrap OR have a body mass greater than 4500 g
```
...
```{r}
penguins %>%
  select(species, body_mass_g) %>% # Select "species" and "body_mss_g" columns
  filter(species=="Gentoo") # Filter for Gentoo penguins
```

Note: select() goes before filter()
