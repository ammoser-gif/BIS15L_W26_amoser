...
```{r}
penguins %>% 
  select(species, flipper_len, body_mass) %>%
  ggplot(mapping=aes(x = body_mass, y = flipper_len)) +
...
```{r}
penguins %>% 
  distinct(species) # Find names of unique species
```

What if we want to know which islands each species occurs on?
```{r}
penguins %>% 
  distinct(species, island, .keep_all=T) # Find unique islands that each species occurs on and keeps the rest of the variables
```
...
```{r}
penguins %>% 
  mutate(body_mass_kg=body_mass/1000, body_mass_g=body_mass) %>%  # Convert body mass from grams to kilograms and add column to end of "penguins" data frame
...
penguins %>% 
  ...
  select(species,
         bill_len_cm=bill_len,
         bill_dep_cm=bill_dep,
         flipper_len_cm=flipper_len)
```
...
```
```{r}
mammals <- read.csv("data/mammal_lifehistories_v2.csv") %>% clean_names() # Try this when inputing new data
```
...
```{r}
mammals %>% 
  mutate(across(c("order", "family", "genus"), tolower))
```

This will change all columns to lower case. But, notice what happens to numeric columns.
```{r}
mammals %>% 
  mutate(across(everything(), tolower)) #Turns everything into lowercase CHARACTER, including NUMERIC variables
```
...
```{r}
mammals_fixed <- mammals %>%
  mutate(across(c(mass, gestation,newborn, weaning, wean_mass, afr, max_life, litter_size, litters_year), ~ifelse(.==-999.00, NA, .)))
```
...
```{r}
mammals_mass_kg <- mammals_fixed %>% 
  select(genus, species, mass) %>% 
  mutate(mass_kg=mass/1000) %>% 
  arrange(desc(mass_kg))
view(mammals_mass_kg)
```

The species with the highest mass is Balaenoptera musculus, aka the Blue Whale.
...
mammals_fixed %>% 
  ...
  geom_smooth(method = "lm") +
  labs(
    title = "Gestation vs. Newborn Mass",
    x = "Gestation Time",
    y = "Newborn Mass (kg)"
  )
```
...
mammals_fixed %>% 
  ...
  mutate(max_life_years=max_life/12) %>% 
  arrange(desc(max_life_years))
```

Balaenoptera physalus (Fin whale) has the longest life span in years.  However, there were a lot of NA's in the Max_life column, so we are not 100% sure.
