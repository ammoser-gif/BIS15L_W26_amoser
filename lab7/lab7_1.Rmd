...
msleep %>% 
  filter(bodywt > 150) %>% 
  summarise(mean_bodywt_large = mean(bodywt))
```
...
msleep %>% 
  filter(bodywt > 100) %>% 
  distinct(genus) %>% # Take only DISTINCT genus, not every observation of a genus
  pull(genus) # Pull all distinct genera as a vector
```
...
```{r}
msleep %>% 
  count(vore) # Vore is x, number of observations is y
```
...
msleep %>% 
  ...
  geom_col() # Use when we have an x AND a y
```
...
```{r}
msleep %>% 
  count(vore, order) # Count observations of each vore-order pair
```
