...
```{r}
heartrate %>% 
  ggplot(x = patient, y =) # Whole table cannot be ploted at once due to margent having four y values
```
...
```
\
Note: `read_csv` includes special characters in variable names but also can turn dbls to characters.
\
2. Why are these data untidy?  
\
\
No religion has their own separate rows of each X.# value
\
\
...
```{r}
billboard_long1 <- billboard %>% 
  pivot_longer(
    wk1:wk76,# Specify columns to pivot
    names_to = "week",
    values_to = "rank",
    values_drop_na = T # Remove all 'NA' values
    )
billboard_long1
```
...
```{r}
billboard_long2 <- billboard %>% 
  pivot_longer(
    -c(artist, track, date.entered),# Specify columns to NOT to pivot
    names_to = "week",
    values_to = "rank",
    values_drop_na = T # Remove all 'NA' values
    )
...
```{r}
billboard_long3 <- billboard %>% 
  pivot_longer(
    cols = starts_with("wk"), # Specify columns to pivot by prefix "wk"
    names_to = "week",
    names_prefix = "wk", # Identify prefix to drop
    values_to = "rank",
    values_drop_na = T # Remove all 'NA' values
    )
billboard_long3
...
\
\
Each observation of genotype does not have its own row.
\
\
...
qpcr_tidy <- qpcr_untidy %>% 
  pivot_longer(
    ...
    names_sep = "_", # Separate name at '_'
...
qpcr_tidy %>% 
  ...
  separate(replicate, into = c("experiment", "e_number"), sep = 3) # %>% 
  select(gene, experiment = e_number, replicate = r_number, "mRNA_expression")
```
